...
import { selectClicksByUTMComparisonOverTime, selectAllLinks, selectUTMContentCounts } from "@/store/selectors/clickSelectors";
...
export function LineChartUTMContent() {
    ...
    const utm_content = useAppSelector(selectUTMContentCounts);


    // Retrieve dynamic time-series data for utm_content using our selector.
    const utmData = useAppSelector((state) =>
        selectClicksByUTMComparisonOverTime(state, "utm_content", primaryValue, comparisonValue)
    );
        ...
        <Card>
            <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-4">
                <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
                    <CardTitle>UTM Content Comparison</CardTitle>
                    <CardDescription>
                    ...
                    <div className="flex gap-2">
                        <Select value={primaryValue} onValueChange={setPrimaryValue}>
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    <SelectLabel>Primary UTM Content</SelectLabel>
                                    {Object.keys(utm_content).map((source) => {
                                        return (
                        ...
                        <Select value={comparisonValue} onValueChange={setComparisonValue}>
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    <SelectLabel>Comparison UTM Content</SelectLabel>
                                    {Object.keys(utm_content).map((source) => {
                                        return (
                                            <SelectItem value={source} key={source}>
                                                {source}
