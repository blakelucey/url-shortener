...
import { selectClicksByUTMComparisonOverTime, selectAllLinks, selectUTMSourceCounts } from "@/store/selectors/clickSelectors";
...
export function LineChartUTMSource() {
    ...
    const utm_source = useAppSelector(selectUTMSourceCounts);


    // Retrieve dynamic time-series data for utm_source using our selector.
    const utmData = useAppSelector((state) =>
        selectClicksByUTMComparisonOverTime(state, "utm_source", primaryValue, comparisonValue)
    );
        ...
        <Card>
            <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-4">
                <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
                    <CardTitle>UTM Source Comparison</CardTitle>
                    <CardDescription>
                    ...
                    <div className="flex gap-2">
                        <Select value={primaryValue} onValueChange={setPrimaryValue}>
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    <SelectLabel>Primary UTM Source</SelectLabel>
                                    {Object.keys(utm_source).map((source) => {
                                        return (
                        ...
                        <Select value={comparisonValue} onValueChange={setComparisonValue}>
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    <SelectLabel>Comparison UTM Source</SelectLabel>
                                    {Object.keys(utm_source).map((source) => {
                                        return (
                                            <SelectItem value={source} key={source}>
                                                {source}
