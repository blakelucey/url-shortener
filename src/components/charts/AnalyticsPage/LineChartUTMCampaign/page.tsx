...
import { selectClicksByUTMComparisonOverTime, selectAllLinks, selectUTMCampaignCounts } from "@/store/selectors/clickSelectors";
...
export function LineChartUTMCampaign() {
    ...
    const utm_campaign = useAppSelector(selectUTMCampaignCounts);

    // Retrieve dynamic time-series data for utm_campaign using our selector.
    const utmData = useAppSelector((state) =>
        selectClicksByUTMComparisonOverTime(state, "utm_campaign", primaryValue, comparisonValue)
    );
        ...
        <Card>
            <CardHeader className="flex flex-col items-stretch space-y-0 border-b p-4">
                <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-5 sm:py-6">
                    <CardTitle>UTM Campaign Comparison</CardTitle>
                    <CardDescription>
                    ...
                    <div className="flex gap-2">
                        <Select value={primaryValue} onValueChange={setPrimaryValue}>
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    <SelectLabel>Primary UTM Campaign</SelectLabel>
                                    {Object.keys(utm_campaign).map((source) => {
                                        return (
                        ...
                        <Select value={comparisonValue} onValueChange={setComparisonValue}>
                            ...
                            <SelectContent>
                                <SelectGroup>
                                    <SelectLabel>Comparison UTM Campaign</SelectLabel>
                                    {Object.keys(utm_campaign).map((source) => {
                                        return (
                                            <SelectItem value={source} key={source}>
                                                {source}
